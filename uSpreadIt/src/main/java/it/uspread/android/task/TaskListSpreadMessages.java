...
/**
 * Tache de chargement asynchrone des message propagé par l'utilisateur
 */
public class TaskListSpreadMessages extends Task<MessageCriteria, Void, List<Message>> {
     ...
     */
    public TaskListSpreadMessages(final int resultCode, final boolean onlyUpdateDynamicValue) {
        super(resultCode);
        this.onlyUpdateDynamicValue = onlyUpdateDynamicValue;
        if (onlyUpdateDynamicValue) {
            listMessageInCacheAtLaunch = new ArrayList<>(USpreadItApplication.getInstance().getMessageCache().getListMessageSpread());
        }
    }

    @Override
    protected List<Message> executeTask(final MessageCriteria... criterias) throws USpreadItException {
        return USpreadItServer.getInstance().getSpreadMessage(criterias[0]);
    }

    @Override
    protected void onFinalizeTask(final List<Message> listMessage) {
        // MAJ du cache RAM
        USpreadItApplication.getInstance().getMessageCache().syncMessagesSpread(listMessage);
