...
/**
 * Tache de chargement asynchrone des message écrits par l'utilisateur
 */
public class TaskListWritedMessages extends Task<MessageCriteria, Void, List<Message>> {
     ...
     */
    public TaskListWritedMessages(final int resultCode, final boolean onlyUpdateDynamicValue) {
        super(resultCode);
        this.onlyUpdateDynamicValue = onlyUpdateDynamicValue;
        if (onlyUpdateDynamicValue) {
            listMessageInCacheAtLaunch = new ArrayList<>(USpreadItApplication.getInstance().getMessageCache().getListMessageWrited());
        }
    }

    @Override
    protected List<Message> executeTask(final MessageCriteria... criterias) throws USpreadItException {
        return USpreadItServer.getInstance().getWritedMessage(criterias[0]);
    }

    @Override
    protected void onFinalizeTask(final List<Message> listMessage) {
        // MAJ du cache RAM
        USpreadItApplication.getInstance().getMessageCache().syncMessagesWrited(listMessage);
